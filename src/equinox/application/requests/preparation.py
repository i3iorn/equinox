...
def build_preflight_issues(
	*,
	...
	if cleaned_url and "{{" not in cleaned_url and not cleaned_url.lower().startswith(("http://", "https://")):
		issues.append(
			PreparationIssue(
				code="url.scheme",
...
def interpolate_request_fields(
	...
	resolved_headers = {
		VariableInterpolator.interpolate(key, merged_vars): VariableInterpolator.interpolate(value, merged_vars)
