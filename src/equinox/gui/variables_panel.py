...
from equinox.storage import Database, EnvironmentManager, VariableGroupManager
...
class VariablesPanel(QWidget):
    ...
    def _build_interp_context(self) -> dict[str, str]:
        ...
        Mirrors the request-time resolution order:
        active environment → OS environment → session vars → path params.
        Each step silently degrades on failure so a broken source does not
        prevent the rest from contributing.
        """
        interp_vars: dict[str, str] = {}

        try:
            active = self._env_mgr.get_active_environment()
            if active:
                interp_vars.update(active.get("variables", {}))
        except Exception as exc:
            logger.debug("Tooltip: failed to fetch active environment: %s", exc)

        try:
            interp_vars.update({
                k: v for k, v in os.environ.items()
                if isinstance(v, str) and _VALID_VAR_NAME.match(k)
            })
        except Exception as exc:
            logger.debug("Tooltip: failed to read OS environment: %s", exc)

        try:
            rp = getattr(self.window(), "request_panel", None)
            if rp is not None:
                interp_vars.update(getattr(rp, "_session_vars", {}))
                path_table = getattr(rp, "path_params_table", None)
                if path_table is not None:
                    interp_vars.update(path_table.get_all_data())
        except Exception as exc:
            logger.debug("Tooltip: failed to read request panel vars: %s", exc)
