...
class PhpCurlGenerator:
    def generate(self, response: Response) -> str:
        ...
        if headers:
            h_list = [f"'{_escape_single_quoted(k)}: {_escape_single_quoted(v)}'" for k, v in headers.items()]
            lines.append(f"$headers = [{', '.join(h_list)}];")
            lines.append("curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);")

        if request.body:
            lines.append(f"curl_setopt($ch, CURLOPT_POSTFIELDS, '{_escape_single_quoted(request.body)}');")
