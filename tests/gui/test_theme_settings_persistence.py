...
These run against the isolated per-test store from the `isolated_gui_settings`
autouse fixture (issue #40), so they never touch the developer's real
appearance settings.
"""
...
def test_font_size_is_readable_immediately_after_writing(isolated_gui_settings):
...
def test_repeated_increments_each_take_effect(isolated_gui_settings):
...
def test_theme_mode_is_readable_immediately_after_writing(isolated_gui_settings):
...
def test_theme_and_ui_common_share_one_settings_handle(isolated_gui_settings):
    ...
    assert _settings() is isolated_gui_settings
