...
def main() -> int:
    ...
    if not ci_tools:
        print("No tools found in CI workflows (might need manual review)")
        return 0

    print(f"[OK] Found {len(ci_tools)} tools in CI workflows:")
    ...
    if not deps["setup.py"]:
        ...
        if deps["pyproject.toml"] != deps["setup.py"]:
            ...
            if pyproject_only:
                print("\nIn pyproject.toml but not setup.py:")
                for pkg in sorted(pyproject_only):
                    print(f"  - {pkg}")

            if setup_only:
                print("\nIn setup.py but not pyproject.toml:")
